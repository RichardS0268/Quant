...
- 假设今天是2015年2月的最后一个交易日：
    ...
    1. 平仓：将目前持仓中不在初选集合里的股票全部平仓（交易价格为Day 2开盘价），并计算平仓后账户现金额
    2. 建仓：
        ...
        3. 计算账户总额 `def Accounting(...)` ，因为我们是月度调仓，因此月末的账户总值=月末小市值股票仓价值（各股票持仓量*月末交易日的收盘价） + 月末大市值股票仓价值（各股票持仓量*月末交易日的收盘价） + 剩余现金
